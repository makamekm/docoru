...
const Language = ({
  ...
  const onLanguage = useCallback((e: any, item: ILanguage) => {
    // e.preventDefault();
    // e.stopPropagation();
    // setOpen(true);
  }, []);
  ...
  return <>
    <Button ref={boxRef} view="flat" size="l" className="!text-black/50" onClick={() => setOpen(true)}>
      <Icon data={Globe} size={20} />
    </Button>
    <Popup
      anchorRef={boxRef}
      placement={"auto"}
      className="max-w-[100%] w-[10rem]"
      hasArrow
      onBlur={onClose}
      onClose={onClose}
      ...
      <div className="p-2 w-full max-w-full max-h-[80vh] overflow-auto">
        <div className="flex flex-col w-full min-w-full max-w-full gap-[1px]">
          {config?.languages?.map((item, index) => {
            ...
            return <a key={item.code ?? index} className={cl("inline px-4 py-2 rounded text-ellipsis min-w-[1px]", {
            ...
            })} onClick={e => onLanguage(e, item)} href={href}>
...
export default function MenuLayout({
  ...
  const search = debounce(async () => {
    const text = searching?.trim()?.toLowerCase();
    if (index && text) {
  ...
  return (
    <div className="flex flex-col min-w-[1px] max-w-full">
      <div className="flex flex-col border-b border-black/10 bg-white h-[4rem] sticky top-0 z-[1] px-3 min-w-[1px] max-w-full">
        <div className="flex-1 flex container mx-auto justify-start items-center gap-2">
          <div className="flex justify-start items-center gap-8 min-w-[1px] max-w-full">
            ...
            {searching == null && <div className="hidden md:flex justify-start items-center gap-6 min-w-[1px] max-w-full overflow-scroll no-scrollbar">
              {items?.map((item, index) => <MenuItem key={index} item={item} />)}
            </div>}
          </div>
          {searching == null && <div className="flex-1"></div>}
          {searching == null && <div className="flex justify-start items-center gap-1">
          ...
          <div className={cl("flex justify-start items-center gap-1", {
                    ...
                    {result?.map(href => {
                      ...
                      return <a key={href} className={cl("flex flex-col gap-2 px-4 py-2 rounded text-ellipsis min-w-[1px]", {
                        ...
                        {!!second && <div className="text-lg">
                          <Highlighter searchWords={query} textToHighlight={second} highlightClassName='bg-purple-200' />
                        </div>}
                      </a>;
                    })}
                  </div>
            ...
            <Button view="flat" size="l" className="!text-black/50" onClick={() => {
              setSearching(value => {
                return value == null ? "" : null;
              });
            }}>
              <Icon data={searching == null ? Magnifier : CircleXmark} size={20} />
            </Button>
          </div>
          <div className="flex lg:hidden">
            <Button ref={boxRef} view="flat" size="l" className="!text-black/50" onClick={() => setOpen(value => !value)}>

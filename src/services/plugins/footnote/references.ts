...
import type { FootnoteRef, Footnotes } from './types'
...
export function createFootnoteRef(prefixId: string, refMarkers = false) {
  ...
  return {
    ...
    renderer({ id, label }: FootnoteRef) {
      ...
      return `<sup class="relative"><a href="#${prefixId + encodedLabel
        }" data-${prefixId}ref aria-describedby="${prefixId}label">${refMarkers ? `[${id}]` : id
        }</a><a id="${prefixId}ref-${encodedLabel}" class="absolute top-[-9.5rem] pointer-events-none"></a></sup>`
    }

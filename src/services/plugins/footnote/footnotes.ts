...
import type { Footnotes } from './types'
    ...
    renderer(this: RendererThis, { raw, items = [] }: Footnotes) {
      ...
      const footnotesItemsHTML = items.reduce(
        (acc, { label, content, refs }) => {
          ...
          refs.forEach((_, i) => {
            footnoteItem += ` <a href="#${prefixId}ref-${encodedLabel}" data-${prefixId}backref aria-label="Back to reference ${label}">${i > 0 ? `↩<sup>${i + 1}</sup>` : '↩'
              }</a>`
          })
      ...
      footnotesHTML += `<span class="relative"><h2 class="sr-only">${raw.trimEnd()}</h2><a id="${prefixId}label" class="absolute top-[-9.5rem] pointer-events-none"></a></span>\n`
